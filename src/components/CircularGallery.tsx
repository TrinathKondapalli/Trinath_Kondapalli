...
class Media {
  ...
  createShader() {
    ...
    if (this.image && this.image.startsWith('http')) {
      ...
      if (ctx) {
        ...
        const gradient = ctx.createRadialGradient(canvas.width, 0, 0, canvas.width, 0, 800);
        gradient.addColorStop(0, 'rgba(109,215,76,0.15)');
        gradient.addColorStop(1, 'transparent');
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        const bottomGlow = ctx.createRadialGradient(canvas.width / 2, canvas.height, 0, canvas.width / 2, canvas.height, 600);
        bottomGlow.addColorStop(0, 'rgba(109,215,76,0.3)');
        bottomGlow.addColorStop(1, 'transparent');
        ctx.fillStyle = bottomGlow;
        ctx.fillRect(0, canvas.height - 300, canvas.width, 300);

        ctx.fillStyle = 'rgba(109,215,76,0.5)';
        ctx.fillRect(canvas.width * 0.1, canvas.height - 4, canvas.width * 0.8, 4);
        ...
        ctx.font = '700 80px Figtree, sans-serif';
        ctx.textAlign = 'left';
        ctx.textBaseline = 'top';
        ctx.fillText(this.text, 72, 220);

        if (this.desc) {
          ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
          ctx.font = '300 48px Figtree, sans-serif';
  ...
  createTitle() {
    if (!this.image || !this.image.startsWith('http')) return; // Skip title for text cards
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
    ...
    const yOffset = 0; // Vertically center the cards
...
interface AppConfig {
  items?: { image: string; text: string; desc?: string; num?: string }[];
  bend?: number;
  textColor?: string;
  borderRadius?: number;
  font?: string;
...
class App {
  ...
  createMedias(
    items: { image: string; text: string; desc?: string; num?: string }[] | undefined,
    bend: number = 1,
    textColor: string,
    borderRadius: number,
    font: string
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    if (distance < 5) {
      ...
      const mouseNormalizedX = (clientX / rect.width) * 2 - 1;
      const mouseWorldX = mouseNormalizedX * (this.viewport.width / 2);
      
      let closestMedia: Media | null = null;
      let minDistance = Infinity;
      this.medias.forEach(media => {
        const dist = Math.abs(media.plane.position.x - mouseWorldX);
        if (dist < minDistance) {
          minDistance = dist;
          closestMedia = media;
        }
  ...
  update() {
    if (!this.isDown && !this.isHovering && Date.now() - this.lastInteractionTime > 3000) {
      this.scroll.target += this.scrollSpeed * 0.5;
    }
  ...
  addEventListeners() {
    ...
    this.boundOnMouseEnter = () => { this.isHovering = true; };
    this.boundOnMouseLeave = () => { this.isHovering = false; this.lastInteractionTime = Date.now(); };
